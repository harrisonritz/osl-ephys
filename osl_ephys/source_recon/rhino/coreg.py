...
def downsample_headshape(
    ...
    Parameters:
    - headshape (numpy.ndarray): Nx3 array of headshape points in meters.
    - include_facial_info (bool): Include facial points if True.
    - remove_zlim (float): Remove points above nasion on the z-axis (meters).
    - method (str): Downsampling method ('gridaverage').
    - facial_info_above_z (float): Max z-value for facial points (meters).
    - facial_info_below_z (float): Min z-value for facial points (meters).
    - facial_info_above_y (float): Max y-value for facial points (meters).
    - facial_info_below_y (float): Min y-value for facial points (meters).
    - facial_info_below_x (float): Min x-value for facial points (meters).
    - downsample_facial_info (bool): Whether to downsample facial points.
    - downsample_facial_info_amount (float): Grid size for downsampling facial info (meters).
    ...
    if include_facial_info:
        ...
        print(facial_points.shape)

        fig = plt.figure(figsize=(12, 6))
        ax = fig.add_subplot(111, projection='3d')
        plot_headshape(headshape_orig, 'red', alpha=0.2, size=2)  # Original
        plot_headshape(facial_points, 'blue', size=10)  # Downsampled
    ...
    if remove_zlim is not None:
        print('Removing Points Below Z_lim')
        # Rotate the point cloud to align the Z-axis with the specified angle
        rotated_headshape = rotate_pointcloud(headshape, angle_degrees, 'x')
        ...
        headshape = rotate_pointcloud(filtered_rotated_points, -angle_degrees, 'x')
    ...
    ax.scatter(headshape[:, 0], headshape[:, 1], headshape[:, 2], c='blye', alpha=1, s=10)
    plot_headshape(headshape, 'blue', size=10)  # Downsampled
    plt.title("Downsampled Headshape")
    plt.show()

    return headshape

def rotate_pointcloud(points, angle_degrees, axis='y'):
    """
